...
export default function VolunteerDashboard() {
  ...
  const sopCards = [
    { type: 'lost_child', title: 'Lost Child', code: 'LC', severity: 'high', color: 'bg-orange-50 border-orange-200 text-orange-700' },
    { type: 'medical', title: 'Medical Emergency', code: 'ME', severity: 'critical', color: 'bg-red-50 border-red-200 text-red-700' },
    { type: 'ticket', title: 'Ticket Issue', code: 'TI', severity: 'low', color: 'bg-yellow-50 border-yellow-200 text-yellow-700' },
    { type: 'crowd', title: 'Crowd Rerouting', code: 'CR', severity: 'medium', color: 'bg-blue-50 border-blue-200 text-blue-700' },
    { type: 'accessibility', title: 'Accessibility Assistance', code: 'AC', severity: 'medium', color: 'bg-green-50 border-green-200 text-green-700' },
  ];
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {sopCards.map(sop => (
                    <button
                      ...
                      className={`${sop.color} border rounded-lg p-5 text-left hover-lift transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 flex flex-col items-start`}
                      aria-label={`${sop.title} SOP card`}
                    >
                      <span className="mb-3 inline-flex h-9 w-9 items-center justify-center rounded-md bg-white/70 border border-current/15 text-xs font-black tracking-wide">{sop.code}</span>
              ...
              <div>
                ...
                {showForm && (
                  <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 shadow-xl border border-gray-100 mb-6 animate-fade-in">
                    ...
                    <button type="submit" disabled={submitting} className="w-full bg-blue-600 hover:bg-blue-700 hover:scale-[1.01] active:scale-[0.99] text-white py-2.5 rounded-xl font-semibold transition-all disabled:opacity-50 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-md shadow-blue-600/10">
                  ...
                  {incidents.map(inc => (
                    <div key={inc.id} className={`bg-white rounded-xl p-5 shadow-sm border border-gray-100 hover:shadow transition-all border-l-4 ${
                      ...
                      {inc.aiSuggestedResponse && (
                        <div className="bg-gradient-to-br from-blue-50 via-indigo-50 to-emerald-50/20 border border-blue-100/50 rounded-xl p-4 shadow-sm mb-3">
                          ...
                          <p className="text-xs text-blue-700/90 leading-relaxed font-medium">{inc.aiSuggestedResponse}</p>
                        </div>
                      )}
