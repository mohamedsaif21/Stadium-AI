      ...
      <main className="flex-1 bg-gray-50">
        <div className="dashboard-band text-white border-b border-navy-800/40">
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <p className="ops-kicker mb-3">Fan route console</p>
            <h1 className="text-3xl font-extrabold mb-1 tracking-tight">Fan Dashboard</h1>
            <p className="text-blue-200/85 text-sm font-medium">Welcome, {user?.name || 'Fan'}. Live routing, accessibility, and transport help are ready.</p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div className="ops-surface rounded-lg p-6">
                ...
                <form onSubmit={handleNavSearch} className="mb-4">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm bg-gray-50/50 hover:bg-gray-50 focus:bg-white transition-all"
                      aria-label="Search stadium locations"
                    />
                    <button
                      type="submit"
                      disabled={navLoading || !navQuery.trim()}
                      className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-semibold transition-all disabled:opacity-50 hover:-translate-y-0.5 active:translate-y-0 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-md shadow-blue-600/15"
                    >
                      {navLoading ? '...' : 'Search'}
                    </button>
                  </div>
                ...
                {navError && <p className="text-red-600 text-xs font-semibold mb-3">{navError}</p>}

                {navResult?.route && (
                  <div className="bg-green-50 border border-green-200/70 rounded-lg p-5 shadow-sm mb-4 animate-fade-in" role="status">
                    <p className="text-green-800 font-bold text-sm mb-3 flex items-center justify-between gap-3">
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {suggestions.slice(0, 4).map(s => (
                    <button
                      ...
                      className="route-chip px-3.5 py-1.5 text-xs text-gray-700 rounded-md hover:-translate-y-0.5 active:translate-y-0 transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 font-semibold"
                    >
                      {s.label}
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {suggestions.map(s => (
                    <button
                      ...
                      className="text-left p-3.5 bg-gray-50/70 hover:bg-blue-50/50 rounded-lg text-xs font-semibold text-gray-700 transition-all border border-gray-100/80 hover:border-blue-200/80 hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      {s.label}
                    </button>
                  ))}
              ...
              <div className="ops-surface rounded-lg p-6">
                ...
                <p className="text-xs text-gray-500 mb-4 leading-relaxed">Use the floating accessibility controls for high contrast and larger text at any time.</p>
